...
/**
 * Synchronized access to native R engine
 * 
 * To use: 1. Install R and rJava using sudo calc/lib/install-R.sh
...
public class R {
	...
	synchronized public void startup() {
		try {
			...
			rStdOutputListner = new RStdOutputListner(this);
			engine = REngine.engineForClass(JRIEngine.class.getName(), R_PARAMS, rStdOutputListner, true);
	...
	private String getJriPath() {
		Scanner s = null;
		System.out.println("Getting JRI path");
		try {
	...
	synchronized REXP eval(String expr, REXP env, boolean resolve, RLogger logger) throws RException {
		try {
			...
			rStdOutputListner.registerLogger(logger);
			...
			rStdOutputListner.unregisterLogger(logger);
		}
	}
